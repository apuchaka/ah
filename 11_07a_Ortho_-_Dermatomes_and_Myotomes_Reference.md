---
block: Musculoskeletal and Orthopaedics
source: quackquackmed 11.07 Dermatomes, Myotomes, and reflexes reference
---

## Dermatomes — quick reference

| Level | Area |
|---|---|
| C2 | Posterior half of the skull ("cap") |
| C3 | High turtleneck shirt |
| C4 | Low-collar shirt |
| C5 | Ventral axial line of upper limb |
| C6 | Thumb + index finger |
| C7 | Middle finger + palm of hand |
| C8 | Ring + little finger |
| T4 | Nipples |
| T5 | Inframammary fold |
| T6 | Xiphoid process |
| T10 | Umbilicus |
| L1 | Inguinal ligament |
| L2, L3 | Anterior & medial thigh |
| L4 | Kneecaps |
| L5 | Big toe, dorsum of foot (except lateral aspect) |
| S1 | Lateral foot, small toe |
| S2, S3 | Genitalia |

> [!tip] Memory aids
> - **C6:** make a "6" with your left hand by touching the tip of the thumb and index finger together.
> - **L4:** "down on all fours" — L4.
> - **L5:** "L5 = Largest of the 5 toes" (big toe).
> - **S1:** "S1 = the smallest one" (small toe).

## Upper limb nerve roots — sensory, motor, and reflexes

> [!note] Gap-filled — genuinely absent despite the equivalent Lower limb table below already existing, and despite cervical radiculopathy (see [[11_06_Ortho_-_Spinal_Orthopaedics]] Cervical spondylosis, not repeated here) being specifically an upper-limb nerve root presentation — the exact scenario this table is most needed for.

| Root | Sensory loss | Motor weakness | Reflexes | Special test |
|---|---|---|---|---|
| C5 | Lateral upper arm (regimental badge area) | Shoulder abduction, elbow flexion | ↓Biceps reflex | — |
| C6 | Lateral forearm, thumb, index finger | Elbow flexion, wrist extension | ↓Brachioradialis reflex (± biceps) | — |
| C7 | Middle finger, palm | Elbow extension, wrist flexion, finger extension | ↓Triceps reflex | — |
| C8 | Ring and little finger, medial forearm | Finger flexion | Reflexes intact | — |
| T1 | Medial upper arm | Finger abduction (intrinsic hand muscles) | Reflexes intact | Consider a Horner's syndrome if a T1 (lower brachial plexus) lesion is proximal enough to involve the sympathetic chain |

## Upper limb peripheral nerve lesions — sensory and motor findings

> [!note] Gap-filled — genuinely absent despite the equivalent Lower limb table below already existing; median, ulnar, and radial nerve lesions are arguably among the most classically tested peripheral nerve lesions in medicine, and no dedicated entry existed anywhere else in this project either (only scattered passing mentions).

| Nerve | Sensory loss | Motor weakness | Special notes |
|---|---|---|---|
| Median (at the wrist — carpal tunnel syndrome) | Palmar aspect of thumb, index, middle, and half of ring finger | Thenar muscle weakness (abductor pollicis brevis) | "Ape hand" deformity in longstanding/severe cases (thenar wasting, thumb falls in line with the fingers); positive Tinel's sign (tapping over the carpal tunnel reproduces symptoms) and Phalen's test (wrist flexion for 60 seconds reproduces symptoms) |
| Median (proximal lesion, e.g. supracondylar fracture) | As above, plus additional forearm sensory loss | As above, plus weakness of the long flexors of the thumb/index/middle fingers and forearm pronation | "Hand of benediction" — on attempted finger flexion (making a fist), the index and middle fingers remain extended while the ring and little fingers flex |
| Ulnar | Little finger and medial half of ring finger | Weakness of the intrinsic hand muscles (interossei, medial two lumbricals), finger abduction/adduction | "Claw hand" (hyperextension at the MCP joints with flexion at the DIP/PIP joints of the ring and little fingers, from unopposed extensor action); positive Froment's sign (weak thumb adduction — patient compensates using flexor pollicis longus, causing thumb IP joint flexion when gripping paper) |
| Radial | Dorsal aspect of the first web space (a small, often the only reliably testable area) | Wrist and finger extension, thumb extension | "Wrist drop" — classically associated with humeral shaft fracture (radial nerve runs in the spiral groove) or prolonged external compression ("Saturday night palsy") |
| Axillary | Small area over the lateral deltoid ("regimental badge" area) | Shoulder abduction (deltoid) | Classic complication of anterior shoulder dislocation or proximal humeral fracture — see [[11_02_Ortho_-_Upper_Limb__Shoulder__Elbow__Distal_Radius_Fractures_]], not repeated here |

## Lower limb nerve roots — sensory, motor, and reflexes

| Root | Sensory loss | Motor weakness | Reflexes | Special test |
|---|---|---|---|---|
| L3 | Anterior thigh | Hip flexion, hip abduction, knee extension | ↓Knee reflex | Positive femoral stretch test |
| L4 | Anterior knee, medial malleolus | Knee extension, hip adduction | Reflexes intact | — |
| L5 | Dorsum of foot | Foot and big toe dorsiflexion | Reflexes intact | Positive sciatic stretch test |
| S1 | Posterolateral leg, lateral foot | Plantarflexion of foot | ↓Ankle reflex | — |

## Peripheral nerve lesions — sensory and motor findings

| Nerve | Sensory loss | Motor weakness | Special notes |
|---|---|---|---|
| Femoral | Anterior, medial thigh and leg | Knee extension, thigh flexion | — |
| Obturator | Medial thigh | Thigh adduction | — |
| Lateral cutaneous (of thigh) | Lateral, posterior thigh | — | → meralgia paraesthetica |
| Tibial | Sole of foot | Plantarflexion & inversion of foot | — |
| Common peroneal | Dorsum of foot + lateral leg | Foot dorsiflexion, eversion | + foot drop |
| Superior gluteal | None | Hip abduction | Positive Trendelenburg sign |
| Inferior gluteal | None | Hip extension, lateral rotation | Associated with sciatic nerve injury; difficulty rising from a seated position |

## Brachial Plexus Injury — Erb's Palsy vs Klumpke's Palsy

> [!note] Gap-filled — genuinely absent as a dedicated entry despite Erb's palsy being mentioned in passing elsewhere in this project (as a neonatal birth injury, see [[16_08-09_Antenatal_and_Perinatal_Problems]], not repeated here) without ever explaining the classic clinical picture or contrasting it with Klumpke's palsy — a natural extension of the C5–T1 nerve root content immediately above.

**The brachial plexus (C5–T1) has an upper and lower portion, and injury to each produces a genuinely distinct, classically-tested clinical picture:**

| | Erb's (Erb–Duchenne) palsy | Klumpke's palsy |
|---|---|---|
| **Roots involved** | Upper plexus — C5–C6 (± C7) | Lower plexus — C8–T1 (± C7) |
| **Mechanism** | Excessive **increase** in the angle between shoulder and neck — forced lateral neck flexion away from the shoulder, or shoulder depression | Excessive **arm abduction** ("hyperabduction traction") — e.g. reaching overhead and being pulled, or breech extraction in obstetric cases |
| **Classic causes** | Shoulder dystocia/difficult vaginal delivery (most common obstetric cause — genuinely the **more common** of the two obstetric brachial plexus injuries, with the better spontaneous-recovery prognosis); motorcycle accidents; traction injuries ("stinger"/"burner" in contact sports) | Rare in obstetric palsy specifically (usually from arm-first/breech presentation with subsequent traction); adult traumatic hyperabduction injuries |
| **Classic posture/deformity** | **"Waiter's tip"** — arm adducted, internally rotated, elbow extended, wrist and fingers flexed (biceps/deltoid/brachialis weakness — "can't lift the arm to wave") | **"Claw hand"** — the intrinsic hand muscles are affected (same appearance as the ulnar/median nerve claw-hand pattern described above, since these lower plexus roots feed those same distal nerves) |
| **Reflex** | Absent biceps reflex (C5–C6) | Reflexes largely preserved (the affected roots don't carry major upper-limb reflexes) |
| **A genuinely important associated finding** | — | **Horner's syndrome** may co-occur if the injury is proximal enough to involve the T1 sympathetic fibres — the same anatomical link already established in the T1 nerve root entry above, not repeated here |

**Ix:** clinical diagnosis based on the pattern of weakness/deformity above; nerve conduction studies/EMG and MRI of the plexus for more detailed localisation or when surgical planning is being considered.

**Mx:** many cases (particularly obstetric Erb's palsy) **recover spontaneously with time** — physiotherapy/occupational therapy to maintain range of motion while awaiting recovery is the mainstay of initial management; surgical exploration and repair (nerve grafting, tendon transfers) is considered for severe injuries or those failing to show adequate recovery by around 3–6 months, given delayed surgical intervention is associated with poorer functional outcomes.
