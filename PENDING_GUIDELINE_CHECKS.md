...
| B30 | `Examination.md` | "Chaperone" vs "observer" terminology | The Medical Board's **revised guidelines use "observer" rather than "chaperone"**. `Examination.md` uses the older term **8 times**. **Not changed** — the claim is snippet-sourced, and "chaperone" is superseded rather than incorrect, so an 8-instance edit on an unverified terminology claim was judged worse than leaving it. Verify against the current Medical Board guideline, then update all 8 together if confirmed. | ⬜ |
...
| Professional Boundaries | `Communication.md` | Medical Board sexual-boundaries guidelines (blocked) | "Observer" terminology change (see B30) |
