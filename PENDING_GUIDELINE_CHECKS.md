...
| B21 | multiple (`18_`, `19_`, `04_Neurology`, `Communication.md`) | Step 10 equity content added in the verification pass | The **aged care eligibility age of 50** is the load-bearing fact across four entries — confirm it is current, since aged care policy is under active reform. The **DFV hospitalisation disparity** is stated as a magnitude, not a number, because published estimates range widely (≈30-fold) and derive partly from older datasets; if a current primary figure is obtainable, replace the range with it. The **dementia prevalence ratios (3.5 at 45–49, 3.8 at 50–54)** come from a single research source read via snippet. | ⬜ |
