...
**Rule 5 quantity pass over Parts A/B/C (2026-08-30) — and a limitation in the integrity pass that preceded it:**

*The limitation, recorded per CLAUDE.md rule 7:* the integrity pass logged above was **purely structural** — tables present, callouts present, links resolve, headers unique. **It never applied CLAUDE.md rule 5 to Parts A/B/C at all.** Every check it ran could pass on a file containing an adult dose that would harm a child, because none of them looked at the content of a number. "Parts A/B/C are clean" was therefore a claim about file structure being read as a claim about clinical safety.

*Scan 1 — absolute quantity figures across all 35 files.* ~51 matches. **3 genuine, 48 dismissed.** Dismissed as reference ranges and diagnostic thresholds rather than administered quantities (haemoglobin g/L, eGFR mL/min/1.73m², SAAG, albumin, urine protein), as trial-arm descriptors (sodium zirconium cyclosilicate 5 g and 10 g arms), as toxicity thresholds rather than prescribing figures (pyridoxine above 1000 mg/day), and as the four already-documented dose exceptions. The **adrenaline** entry was re-checked specifically against the historical "band that stopped at 7.5 kg" defect: it is **correct** — the ampoule line covers all ages at 0.01 mL/kg to a 0.5 mL cap, so a child below the injector band floor is covered, and the entry explicitly warns against converting the dose to a single absolute figure.

Genuine findings, one commit each:
- `NEW_Drugs_10_Endocrine.md` — **hydrocortisone 100 mg IV at induction and 200 mg per 24 hours** stated as bare absolute figures for perioperative cover. Adult doses; a large multiple of a small child's requirement. Flagged adult-only with the correct paediatric form named (weight- or surface-area-based) but **no paediatric figure invented** (commit `8184fc8`).
- `NEW_Investigations_Endocrine.md` — the **75 g OGTT load**, which reads as the *name* of the test and is why it survived earlier passes. Marked as the adult and pregnancy load; paediatric load is per-kg with a 75 g cap (commit `edccf6f`).
- `NEW_Investigations_Respiratory.md` — the **≥200 mL** component of the bronchodilator reversibility criterion, an adult absolute that fails at small lung volumes. Noted; the recorded source conflict left intact and still unresolved (commit `bc0db9c`).

*Scan 2 — defensive re-scan for the first scan's blind spots (per rule 4): markdown emphasis between number and unit (`**100** mg`), unicode dose units (µg/μg), and quantities written as words.* The first two blind spots were real — the original pattern could not have matched either. 4 matches, **1 genuine, 3 dismissed.**
- `NEW_Investigations_General_and_Preventive.md` — 24-hour urinary copper given as **>100 µg/day with no paediatric qualifier**, while `NEW_Investigations_Renal_and_Urology.md` 0.11 states both the paediatric (>40 µg/day) and adult figures for the same test. The paediatric value was the one that went missing. Added, with the cross-reference (commit `5e25810`). **A wrong section number was written into that cross-reference first (0.1, which is Urinalysis Panel) and caught by verifying the target heading before commit — rule 1 working as intended.**

**This pass found 4 genuine content defects that the structural pass could not have found. It is a new technique, not a re-run, which is the only reason it found anything.**
